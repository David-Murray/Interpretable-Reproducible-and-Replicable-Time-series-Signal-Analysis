...
def get_arguments():
    ...
    parser.add_argument('--blank',
                        type=int,
                        default=None,
                        help='specify_mask_start')
    parser.add_argument('--blanksize',
                        type=int,
                        default=None,
...
if args.transfer:
    ...
    print('arg.transfer'.format(args.transfer))
    param_file = args.trained_model_dir+'/cnn_s2p_' + args.appliance_name + '_pointnet_model'+ args.specific
else:
    print('arg.transfer'.format(args.transfer))
...
# aggregate_mean = 522 # REFIT-agg
# aggregate_std = 814 # REFIT-agg
aggregate_mean = 281 # eco-Agg
aggregate_std = 509 # eco-Agg


appliance_mean = params_appliance[args.appliance_name]['mean']
appliance_std = params_appliance[args.appliance_name]['std']
appliance_mean = 415 # eco-WM
appliance_std = 698 # eco-WM
...
# Numpy saving
np.save(save_name + '_pred' + args.specific + '_' + str(args.blank) +'.npy', savepred)
np.save(save_name + '_gt' + args.specific + '_' + str(args.blank) +'.npy', savegt)
np.save(save_name + '_mains' + args.specific + '_' + str(args.blank) +'.npy', savemains)
...
if args.plot_results:
    ...
    if args.plot_results:
        ...
        ax1.set_ylabel('W')
        ax1.legend(['aggregate', 'ground truth', 'prediction'], loc='upper left')
